...
analytic_df <- readRDS(paste0(path_grady_hiv_cascade_folder,"/working/cleaned/mhhgra02_analytic sample.RDS")) %>% 
  ...
  mutate(monitored_in_cp1 = case_when(cp1 == 1 ~ monitored,
         ...
         control_in_monitored = case_when(monitored == 1 ~ control,
                                          TRUE ~ NA_real_)) %>% 
  mutate(treat_in_cp1 = case_when(cp1 ==1 ~ treat,
                                  TRUE ~ NA_real_),
         control_in_cp1 = case_when(cp1 == 1 ~ control,
                                    TRUE ~ NA_real_))
...
estimates_in_total = 
  ...
  summarize(a1 = mean(cp1),
            a2 = mean(monitored),
            a3 = mean(treat),
            a4 = mean(control),
            b2 = mean(monitored_in_cp1,na.rm=TRUE),
            b3 = mean(treat_in_monitored,na.rm=TRUE),
            b4 = mean(control_in_monitored,na.rm=TRUE),
            c3 = mean(treat_in_cp1,na.rm=TRUE),
            c4 = mean(control_in_cp1,na.rm=TRUE)) %>% 
  ungroup()
...
counts_in_total = 
  ...
  group_by(stratification) %>% 
  summarize(a1 = sum(!is.na(cp1)),
            a2 = sum(!is.na(monitored)),
            a3 = sum(!is.na(treat)),
            a4 = sum(!is.na(control)),
            b2 = sum(!is.na(monitored_in_cp1)),
            b3 = sum(!is.na(treat_in_monitored)),
            b4 = sum(!is.na(control_in_monitored)),
            c3 = sum(!is.na(treat_in_cp1)),
            c4 = sum(!is.na(control_in_cp1))) %>% 
  ungroup()
...
#   ) %>% 
#   group_by(stratification) %>% 
#   summarize(a1 = sum(cp1),
#             a2 = sum(monitored),
#             a3 = sum(treat),
#             a4 = sum(control),
#             b2 = sum(monitored_in_cp1,na.rm=TRUE),
#             b3 = sum(treat_in_monitored,na.rm=TRUE),
#             b4 = sum(control_in_monitored,na.rm=TRUE),
#             c3 = sum(treat_in_cp1,na.rm=TRUE),
#             c4 = sum(control_in_cp1,na.rm=TRUE)) %>% 
#   ungroup()
...
write_csv(fig_df,"hypertension/mhhgra05_figures care cascade.csv")

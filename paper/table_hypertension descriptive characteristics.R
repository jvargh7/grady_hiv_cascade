...
mhhgra07_out_stratified <- read_csv("hypertension/mhhgra07_descriptive characteristics by rasegrp.csv") %>% 
  mutate(selected = case_when(est == "missing" ~ 1,
                              variable %in% c("bmi","hdl","ldl","tgl","glucose","sbp","dbp") & est %in% c("mean","sd") ~ 1,
                              variable %in% c("hba1c","alt","ast") & est %in% c("median","q25","q75") ~ 1,
                              variable == "is_black" ~ 1,
  ...
  dplyr::filter(selected == 1) %>% 
  dplyr::select(rasegrp,variable,group,est,value,type) %>% 
  mutate(value_cleaned = case_when(est == "proportion" ~ paste0(round(value,1),"%"),
                                   TRUE ~ as.character(round(value,1)))) %>% 
  mutate(variable = case_when(est == "missing" ~ paste0(variable,"_","missing"),
                              TRUE ~ variable)) %>% 
  mutate(est = factor(est,levels=c("mean","sd","median","q25","q75","freq","proportion"))) %>% 
  arrange(rasegrp,variable,group,est) %>% 
  group_by(rasegrp,variable,group) %>% 
  summarize(output = paste0(value_cleaned,collapse = " [")) %>% 
  ungroup() %>% 
  pivot_wider(names_from=rasegrp,values_from=output) %>% 
  dplyr::select(variable,group,everything())

write_csv(mhhgra07_out_stratified,"paper/table_hypertension descriptive characteristics by rasegrp.csv")

...
analytic_df = dr_hussen_cohort_0216 %>% 
  mutate(detected = case_when(mrn %in% cp2$mrn ~ 1,
                         TRUE ~ 0)) %>% 
  left_join(sexual_orientation %>% 
  ...
  left_join(cp2 %>% 
            ...
            by = "mrn") %>% 
  mutate(cp2 = case_when(!is.na(contact_date) ~ 1,
                         TRUE ~ 0)) %>% 
...
saveRDS(analytic_df,paste0(path_grady_hiv_cascade_folder,"/working/cleaned/ghcd02_analytic sample.RDS"))
